...
def reasoner_node(state: AgentState):
    ...
    10. 🚫 LaTeX 수식 문법($$...$$, \\text{{}}, \\frac{{}}{{}}, \\left( 등) 절대 사용 금지. 수식이나 계산식은 반드시 일반 텍스트로 표현하세요. (예: 지급액 = (단가 ÷ 기준연비) × 실제거리)
    11. 답변을 모두 작성한 후, 맨 마지막 줄에 사용자가 이어서 궁금해할 만한 '추천 질문' 3가지를 반드시 "|||SUGGESTIONS|||" 이라는 구분자 뒤에 줄바꿈으로 구분하여 작성하세요.
...
def bq_node(state: AgentState):
    ...
    try:
        ...
        - 🚫 LaTeX 수식 문법($$...$$, \\text{{}}, \\frac{{}}{{}}, \\left( 등) 절대 사용 금지. 계산식은 일반 텍스트로 표현하세요. (예: 지급액 = 단가 ÷ 기준연비 × 실제거리)
...
def email_read_node(state: AgentState):
    ...
    try:
        ...
        # 메일 메타데이터 병렬 수집
        # httplib2.Http는 thread-safe하지 않으므로 gmail 객체 공유 대신
        # requests.get() 직접 호출 (각 호출이 독립 TCP 연결 사용)
        from concurrent.futures import ThreadPoolExecutor
        import requests as _req
        _token = gmail._http.credentials.token

        def _fetch_msg(msg_info):
            resp = _req.get(
                f"https://gmail.googleapis.com/gmail/v1/users/{user_email}/messages/{msg_info['id']}",
...
[우선순위 규칙 — 최대 10건만 반환]
위 포함 기준을 충족하는 메일이 10건을 초과하면, 아래 순서로 상위 10건만 선별하세요:
① 마감기한이 가장 임박한 것 (detected_due_date 기준)
② 결재·처리·승인 요청
③ 답변·확인·신청이 필요한 메일
④ 중요 업무 변경·공지
...
def drive_search_node(state: AgentState):
    ...
    if not any(kw in user_input for kw in DRIVE_TRIGGER_KEYWORDS):
        ...
        return {"messages": [AIMessage(content=(
            ...
            "AI 챗봇의 **사내 지식베이스(RAG)**는 공식 등록된 규정 문서 기반으로 답변드립니다.\n"
